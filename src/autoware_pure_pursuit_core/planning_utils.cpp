...
{
/**
 * @description: 计算从源索引到目标索引之间的路径弧长
 * @param {Trajectory &} input_path：输入路径
 * @param {size_t} src_idx：源索引
 * @param {size_t} dst_idx：目标索引
 * @return {*}
 */
...
/**
 * @description: 计算目标点到线段的垂直距离
 * @param {Point &} line_s：线段终止点
 * @param {Point &} line_e：线段起始点
...
/**
 * @description: 计算最终曲率
 * @param {Point &} target：预瞄点
 * @param {Pose &} current_pose：车辆位姿
 * @return {*}
 */
double calcRadius(
  const geometry_msgs::msg::Point & target, const geometry_msgs::msg::Pose & current_pose)
...
/**
 * @description: 将曲率转换为转向角度
 * @param {double} wheel_base：轮距
 * @param {double} kappa：曲率
 * @return {*}
 */
...
/**
 * @description: 在给定的路径点集合中找到与当前车辆姿态和距离最接近的点的索引
 * @param {vector<geometry_msgs::msg::Pose> &} poses：路径点
 * @param {Pose &} current_pose：当前车辆位姿
 * @param {double} th_dist：距离阈值
 * @param {double} th_yaw：偏航角阈值
 * @return {*}
 */
...
/**
 * @description: 将一个origin点的相对坐标系中的目标点转换到绝对坐标系中
 * @param {Point &} point：目标点
 * @param {Pose &} origin：相对坐标系原点
 * @return {*}
 */
...
/**
 * @description: 绝对坐标系中的目标点转换到origin点的相对坐标系中
 * @param {Point &} point：目标点
 * @param {Pose &} origin：相对坐标系原点
 * @return {*}
 */
